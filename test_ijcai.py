...
def validate(valid_loader_sketch, valid_loader_image, net, epoch, best, cfg):
    ...
    if best==True:
        if cfg.dataset == 'Sketchy':
            np.save("./danbu_sketchy_acc_sk_em.npy", acc_sk_em)
            np.save("./danbu_sketchy_acc_cls_sk.npy", acc_cls_sk)
            np.save("./danbu_sketchy_acc_im_em.npy", acc_im_em)
            np.save("./danbu_sketchy_acc_cls_im.npy", acc_cls_im)
        if cfg.dataset == 'TU-Berlin':
            np.save("./danbu_Berlin_acc_sk_em.npy", acc_sk_em)
            np.save("./danbu_Berlin_acc_cls_sk.npy", acc_cls_sk)
            np.save("./danbu_Berlin_acc_im_em.npy", acc_im_em)
            np.save("./danbu_Berlin_acc_cls_im.npy", acc_cls_im)
    # Compute mAP
    ...
    aps200 = misc.apsak(sim_euc, str_sim, k=200)
    prec100, _ = misc.precak(sim_euc, str_sim, k=100)
    prec200, _ = misc.precak(sim_euc, str_sim, k=200)

    apsall_bin = misc.apsak(sim_bin, str_sim)
    aps200_bin = misc.apsak(sim_bin, str_sim, k=200)
    prec100_bin, _ = misc.precak(sim_bin, str_sim, k=100)
    prec200_bin, _ = misc.precak(sim_bin, str_sim, k=200)
